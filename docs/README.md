...
> **상태**: 설계 단계. **코드는 아직 없음.** 이 세트는 Ralph loop(자동 반복 개선) 투입 직전까지의 산출물이다.
...
- [ ] 사용자+AI 공동 채점표 검토
- [ ] (이후) Ralph loop — **vertical slice 6종** 통과를 게이트로: ①법령MCP 앵커 답변 ②citation 검증된 RAG 답변 ③공식소스 Web run ④충돌 케이스 ⑤CPA 검토 워크플로 ⑥테넌트격리 테스트
