...
선택지 비교표 · 리스크 대시보드 · 리서치 메모 · 증빙 인덱스 · **DOCX 검토패키지(13목차)** · **시나리오 Tax Plan 보고서**(재무제표 쟁점 강조 + 그림·도표 + 근거 하이퍼링크, 회계사 자연어) · 회계사 검토항목 리스트.
...
| `OUT-009` | **시나리오 Tax Plan(실행계획) 보고서** 생성 | **재무상태표·손익계산서 첨부 + 쟁점 계정행 음영 강조·쟁점 라벨·근거 링크 + 시나리오별(현금배당·유상감자·혼합) 세부담/세액계산 비교 + 권고안 + 실행 타임라인 + 가업승계 증여세. 인용은 표시 전 전수 검증(미검증 시 생성 차단=fail-closed), 권고 시나리오 ≥1. 구현=`src/tax_plan*.py`** | P1 |
| `OUT-010` | **회계사 자연어 렌더링 계층**(영어·개발어 0 + 근거 하이퍼링크 + 그림·도표 + 폰트) | **최종 reviewer(KICPA)용 산출물에 영문/식별자 토큰 0(자연어화 헬퍼) · 근거는 국가법령정보센터 하이퍼링크 · 가독성 그림 4종(matplotlib) · Noto Sans KR. `OUT-009`/검토패키지 자연어 보고서(`src/cpa_report.py`) 공통** | P1 |
